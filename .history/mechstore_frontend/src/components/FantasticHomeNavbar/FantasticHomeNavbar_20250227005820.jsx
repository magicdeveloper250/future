        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center space-x-4">
            ...
            <Link to="/" className="flex items-center">
              <img src="/logo/logo.svg" alt="Future Electronics logo"   className="h-20 w-auto" />
            </Link>
          </div>
          ...
          <ul className="links-container flex justify-center space-x-8 h-12 items-center">
            {links.map(({ id, label, icon }) => (
              <li key={id} className="link-item">
                <Link 
                  to={`#${id}`}
                  className={`flex items-center space-x-2 py-2 rounded-md text-center ${activeLink === id ? `${isScrolled ? "text-black bg-slate-200" : "text-slate-200 bg-black"} font-medium`: "hover:opacity-80"}`} 
