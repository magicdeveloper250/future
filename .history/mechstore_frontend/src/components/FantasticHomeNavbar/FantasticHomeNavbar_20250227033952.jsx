          ...
          <ul className="links-container flex justify-center space-x-8 h-12 items-center">
            {links.map(({ id, label, icon }) => (
              <li key={id} className="link-item">
                <Link 
                  to={`#${id}`}
                  className={`flex items-center space-x-2 py-2 rounded-md text-center px-2 ${activeLink === id ? `${isScrolled ? "text-black bg-sky-500" : "text-slate-200 bg-red-500"} font-medium`: "hover:opacity-80"}`} 
