      ...
      <div className="absolute inset-0">
        <div className="container mx-auto px-6 h-full flex items-center">
          ...
          <div className="hidden lg:block w-1/2">
            ...
            <div 
                className="absolute inset-0 rounded-2xl overflow-hidden shadow-2xl transform transition-transform hover:scale-105 duration-500 ease-in-out grid grid-cols-4 grid-rows-4 gap-4"
                style={{ 
                  boxShadow: "0 10px 30px -5px rgba(0, 0, 0, 0.3)",
                }}
              >
                { products?.filter((product) => product.categoryId === categories[currentCategory]?.categoryId).map((product, index) => {
