...
const ShowProductsByCategoryId = (req,res) => {
    const {categoryId} = req.params;
    ProductModel.SelectProductsByCategoryId(categoryId,(err,results) => {
      if(err) return res.status(500).json(err.message);
      return res.status(200).json({
        responseCode:201,
        products:results
...
,RemoveCategories,ShowCategories,CreateCategories,ShowCategoriesById, DeleteCategory, updateMessageStatus, bestSellingProduct, todayDeals,ShowProductsByCategoryId };